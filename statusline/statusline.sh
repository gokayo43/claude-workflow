#!/bin/bash
# Claude Code status line — adapted from @kamranahmedse/claude-statusline for Windows/Git Bash
# Original: https://github.com/kamranahmedse/claude-statusline
# Changes: replaced BSD date with GNU date, removed macOS keychain, fixed paths for Windows
set -f

# Ensure jq is on PATH (winget installs it outside Git Bash's default PATH)
export PATH="$PATH:$HOME/AppData/Local/Microsoft/WinGet/Packages/jqlang.jq_Microsoft.Winget.Source_8wekyb3d8bbwe"
...
# Convert ISO 8601 timestamp to epoch seconds (GNU date version)
iso_to_epoch() {
    local iso_str="$1"

    # GNU date handles ISO 8601 directly with -d
    local epoch
...
# Format a reset time from ISO string (GNU date version)
format_reset_time() {
    local iso_str="$1"
    local style="$2"
...
line1+="${sep}"
line1+="${pct_color}ctx ${pct_used}%${reset}"
line1+="${sep}"
...
get_oauth_token() {
    ...
    # Read from credentials file (primary method on Windows)
...
if [ -f "$cache_file" ]; then
    # GNU stat uses -c %Y for modification time
...
if [ -n "$usage_data" ] && echo "$usage_data" | jq -e . >/dev/null 2>&1; then
    ...
    if [ "$extra_enabled" = "true" ]; then
        ...
        # Next month's first day (GNU date)
